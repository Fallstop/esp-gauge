...
The inspector's PWM label is a 30px mint monospace anchor, with a short entrance on port changes (disabled for reduced motion). The source chooser uses category-first navigation with global search: a flat grouped list was too long, and tabs would compress category names in the narrow sidebar. The selected source retains its provider/category above its name. Source-specific configuration appears directly below it.

Update status lives below the PCB, including throughout bootloader disconnection and restart. Download/write progress is measured, while connection and verification show an indeterminate indicator and elapsed time. Completion/error remains until dismissed. During flashing, header labels disappear and the board illustration leaves space for the status.

<script lang="ts">
  import { inverseCurve } from './model';
  let {
    position = 0,
...
<svg class="dial" class:large viewBox="0 0 300 170" role="img" aria-label={label}>
  ...
  {/each}
  <text x="29" y="109">{min}</text><text x="150" y="43" text-anchor="middle"
    >{Number((min + (max - min) * inverseCurve(0.5, curve)).toFixed(1))}</text
  ><text x="264" y="109" text-anchor="middle">{max}</text>

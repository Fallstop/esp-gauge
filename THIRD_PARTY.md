...
Hardware and sound sources use CPAL (Apache-2.0), volumecontrol (MIT), nvml-wrapper (MIT/Apache-2.0), macpow (MIT), wmi (MIT/Apache-2.0), and Core Foundation bindings (MIT/Apache-2.0), plus their lockfile-pinned native bindings. These providers use OS/driver interfaces; no vendor driver is bundled for GPU or temperature monitoring.
